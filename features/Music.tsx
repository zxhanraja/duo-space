...
export const MusicPlayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const killPlaylist = syncService.subscribe('playlist_update', (p: Song[]) => {
      setQueue(p || []);
    });
  ...
  const reconcilePlayer = (remote: PlayerState) => {
    ...
    // Ensure iframe is ready before sending commands
    const commandDelay = 800;
    setTimeout(() => {
      if (remote.isPlaying) {
        sendYTCommand('playVideo');
        sendYTCommand('seekTo', [syncedProgress, true]);
        // On mobile, sometimes we need to double-trigger play due to user gesture requirements
        sendYTCommand('playVideo');
      } else {
  ...
  const handleToggle = () => {
    ...
    // Direct local feedback for better UX
    if (isNowPlaying) {
      sendYTCommand('playVideo');
    } else {
      sendYTCommand('pauseVideo');
  ...
  const addToSpace = () => {
    ...
    if (!playerState.currentSongId || !playerState.isPlaying) {
  ...
  return (
    <GlassPanel className="h-full flex flex-col min-h-0 border-none shadow-none p-0 overflow-hidden" title="AUDIO NODE 🎧">
      {/* Off-screen but active iframe for better mobile behavior */}
        ...
        {queue.map(s => (
          ...
          >
            <div className="relative shrink-0">
              <img src={s.coverUrl} className={`w-6 h-6 rounded object-cover border border-current/5 ${activeSong?.id === s.id ? '' : 'grayscale opacity-30'}`} alt="thumb" />
            </div>
