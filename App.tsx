...
const DuoSpaceShell: React.FC<{ user: User }> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const unsubNudge = syncService.subscribe('nudge_event', () => {
      setIsShaking(true); if (window.navigator.vibrate) window.navigator.vibrate([200, 100, 200]);
  ...
  const logout = () => {
    if (confirm("Logout current user?")) {
      try { localStorage.removeItem('duospace_v5_active_user'); } catch(e) {}
      window.location.reload();
    }
  };

  return (
    <div className={`fixed inset-0 w-full h-[100dvh] flex flex-col ${theme.bgGradient} ${theme.textColor} overflow-hidden font-mono ${isShaking ? 'animate-shake' : ''}`}>
      {/* HEADER */}
      <header className={`shrink-0 px-4 py-2 border-b-2 ${theme.borderColor} flex items-center justify-between z-[100] bg-inherit/95 backdrop-blur-xl`}>
        <div className="flex items-center gap-2 cursor-pointer group" onClick={logout}>
          <div className="relative shrink-0">
            <img src={user.avatar} className={`w-8 h-8 rounded-full border-2 ${theme.borderColor} shadow-sm transition-transform group-hover:scale-110`} alt="U" />
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => syncService.sendShake()} className={`w-7 h-7 flex items-center justify-center bg-current/5 rounded-lg border-2 ${theme.borderColor} hover:bg-current/10 transition-all text-xs`}> 🔔 </button>
          <button onClick={cycleTheme} className={`w-7 h-7 flex items-center justify-center bg-current/5 rounded-lg border-2 ${theme.borderColor} hover:bg-current/10 transition-all text-xs`}> {theme.id === 'dark' ? '☀️' : '🌙'} </button>
          <button onClick={() => setShowPairDialog(true)} className={`w-7 h-7 flex items-center justify-center bg-current/5 rounded-lg border-2 ${theme.borderColor} hover:bg-current/10 transition-all text-xs`}> 🔗 </button>
        </div>
      </header>

      {/* CORE WRAPPER */}
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden relative">
        <nav className={`shrink-0 md:w-16 flex md:flex-col items-center justify-around md:justify-center py-2 md:py-0 gap-6 border-t-2 md:border-t-0 md:border-r-2 ${theme.borderColor} bg-current/[0.02] backdrop-blur-lg z-50 order-3 md:order-1`}>
          {[
            { id: 'home', icon: '🏠' }, 
            { id: 'notes', icon: '📝' }, 
            { id: 'canvas', icon: '🎨' }, 
            ...
            >
              <div className={`w-10 h-10 flex items-center justify-center rounded-2xl border-2 transition-all ${view === v.id ? 'bg-white text-black border-white shadow-[0_0_15px_rgba(255,255,255,0.3)] scale-110' : `${theme.borderColor} bg-transparent`}`}>
...
const Login: React.FC<{ onEntry: (u: User) => void }> = ({ onEntry }) => (
  <div className="h-[100dvh] w-full flex items-center justify-center bg-black p-8 font-mono text-white overflow-hidden relative">
    ...
    <div className="w-full max-w-sm text-center z-20 animate-in slide-in-from-bottom-10 duration-700">
      <div className="inline-block px-5 py-2 bg-white text-black text-[9px] font-black uppercase tracking-[0.6em] mb-12 rounded-full shadow-lg">DUO_SPACE_V5.0</div>
      <h1 className="text-8xl font-black mb-16 uppercase italic tracking-tighter leading-[0.7] text-white">DUO<br/><span className="text-outline">SPACE</span></h1>
      <div className="space-y-4">
        {USERS.map(u => (
          <button 
            key={u.id} 
            onClick={() => onEntry(u)} 
            className="w-full flex items-center gap-6 p-5 border-2 border-white/10 hover:border-white transition-all group active:scale-95 rounded-3xl bg-white/5 hover:bg-white/10 relative overflow-hidden"
          >
            <div className="absolute inset-0 bg-white translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-300 -z-10"></div>
            ...
            <span className="ml-auto text-3xl group-hover:text-black opacity-30 group-hover:opacity-100">→</span>
          </button>
        ))}
      </div>
      <style>{`.text-outline { -webkit-text-stroke: 2px white; color: transparent; }`}</style>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const savedUserId = localStorage.getItem('duospace_v5_active_user');
      if (savedUserId) {
  ...
  if (isLoading) return <div className="h-[100dvh] bg-black"></div>;
  if (!currentUser) return <ThemeProvider><Login onEntry={handleEntry} /></ThemeProvider>;
  return <ThemeProvider><DuoSpaceShell user={currentUser} /></ThemeProvider>;
};
